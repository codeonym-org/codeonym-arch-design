# OBS setup for this machine

OBS stores everything in `~/.config/obs-studio/`:
...
| Recording → Type / format | Standard · **MKV** · path `~/Videos/OBS` |
| Recording → Encoder | **FFmpeg VAAPI H.264** (device `/dev/dri/renderD128`) |
| Rate control | **CQP**, QP **18** (screen text stays sharp) · keyframe interval 2 s · profile High |
...
|---|---|
| **Screen** | Screen Capture (PipeWire, cursor: embedded) |
| **Screen + Cam** | Webcam (V4L2) with *Advanced Mask* rounded rect + *Stroke Glow Shadow*, bottom-right 360×360 · Screen Capture |
...
- **Keystrokes**: run `showmethekey-gtk`, add a *Window Capture (PipeWire)* of its overlay window, place bottom-center.
...
## Hotkeys (Settings → Hotkeys, global via obs-wayland-hotkeys)
| Action | Suggested |
|---|---|
...
GNOME may ask once to allow OBS to register global shortcuts — accept.
