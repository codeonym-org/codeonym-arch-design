...
```bash
bash scripts/blender.sh -b --factory-startup -P scripts/mockup.py -- \
...
## 4. Low-poly scene
Icosphere (subdivisions 1–2) rocks, Cone trees, a Grid plane with *Displace* modifier (Clouds texture) for terrain → flat shading (right-click → Shade Flat) → simple palette materials (use `ui-design` → `palette.sh` colors) → Sun + Sky → EEVEE with Bloom-like glare in the compositor.
...
2. Export:
   ```bash
   bash scripts/convert-model.sh hero.blend public/models/hero.glb --draco
   ```
3. In the app:
   ```bash
...
3. Apply all modifiers, then `bash scripts/convert-model.sh part.blend part.stl`.

## 7. Portfolio turntable of any downloaded model
```bash
bash scripts/blender.sh -b --factory-startup -P scripts/turntable.py -- --model model.glb --frames 180 --res 1920x1080 --save model-tt.blend
bash scripts/render.sh model-tt.blend --anim --out renders
```
