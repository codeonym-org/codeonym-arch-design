...
# Animations render PNG frames, then encode an MP4 with /usr/bin/ffmpeg (VAAPI when available).
set -euo pipefail
. "$(dirname "$(readlink -f "$0")")/../../../lib/env.sh"
blend=${1:-}; [[ -f $blend ]] || { sed -n '2,5p' "$0"; exit 2; }
shift
...
if ((anim)); then
  ...
  enc=("${sw[@]}")
  if [[ -e /dev/dri/renderD128 ]] && [[ $(/usr/bin/ffmpeg -hide_banner -encoders 2>/dev/null) == *h264_vaapi* ]]; then
    enc=(-vaapi_device /dev/dri/renderD128 -vf 'format=nv12,hwupload' -c:v h264_vaapi -qp 18)
  fi
  encode() {
    /usr/bin/ffmpeg -y -loglevel error -framerate "${fps:-24}" -pattern_type glob -i "$out/${name}_*.png" \
