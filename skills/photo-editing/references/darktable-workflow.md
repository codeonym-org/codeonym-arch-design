...
3. *lighttable → styles → create* — tick only the look modules (color balance rgb, sigmoid, etc.), not crop/exposure. Name it e.g. `codeonym-portfolio`.
4. Apply to others in the GUI (select → click the style), or headless:
   ```bash
   bash scripts/batch-develop.sh codeonym-portfolio shoot/*.jpg
   ```
