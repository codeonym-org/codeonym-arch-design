...
- **Proxy clips**: *Project Settings → Proxy → Enable* (codec: x264-vaapi if offered, otherwise default). Toggle on the timeline with the proxy button. Essential for smooth 4K/long recordings on the Vega iGPU.
...
3. After export, measure/fix loudness: `bash scripts/loudnorm.sh exports/final.mp4 --target -14`.
...
- Or render from the terminal: `bash scripts/render-project.sh video-name.kdenlive exports/video-name.mp4`.
- Then platform versions: `bash scripts/encode.sh exports/video-name.mp4 vertical` etc.
