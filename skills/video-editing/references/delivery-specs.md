...
## ffmpeg one-liners (use `/usr/bin/ffmpeg`)
```bash
# Lossless trim (cuts on keyframes; fast)
/usr/bin/ffmpeg -ss 00:01:05 -to 00:02:30 -i in.mp4 -c copy trimmed.mp4

# Frame-accurate trim (re-encodes)
/usr/bin/ffmpeg -ss 00:01:05.200 -to 00:02:30.500 -i in.mp4 -c:v libx264 -crf 18 -c:a aac out.mp4

# Concatenate clips with identical codecs
printf "file '%s'\n" part1.mp4 part2.mp4 part3.mp4 > list.txt
/usr/bin/ffmpeg -f concat -safe 0 -i list.txt -c copy joined.mp4

# Replace / add music bed at -20 dB under original audio
/usr/bin/ffmpeg -i video.mp4 -i music.mp3 -filter_complex "[1:a]volume=-20dB[m];[0:a][m]amix=inputs=2:duration=first[a]" -map 0:v -map "[a]" -c:v copy out.mp4

# Burn subtitles (styled)
/usr/bin/ffmpeg -i in.mp4 -vf "subtitles=subs.srt:force_style='FontName=Inter,FontSize=22,Bold=1,Outline=2'" -c:a copy out.mp4

# Speed up a boring section 4× (video + audio)
/usr/bin/ffmpeg -i in.mp4 -filter_complex "[0:v]setpts=PTS/4[v];[0:a]atempo=2,atempo=2[a]" -map "[v]" -map "[a]" fast.mp4

# Extract audio for a podcast
/usr/bin/ffmpeg -i in.mp4 -vn -c:a libmp3lame -q:a 2 audio.mp3

# Add a logo watermark bottom-right
/usr/bin/ffmpeg -i in.mp4 -i logo.png -filter_complex "[1]scale=160:-1,format=rgba,colorchannelmixer=aa=0.6[l];[0][l]overlay=W-w-32:H-h-32" -c:a copy out.mp4

# Inspect a file
/usr/bin/ffprobe -v error -show_entries stream=codec_name,width,height,r_frame_rate,bit_rate -of compact in.mp4
```
