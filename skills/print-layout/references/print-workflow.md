...
```bash
bash scripts/new-print-doc.sh a5 event-flyer --title "Agents that level up" --font "IBM Plex Sans" --proof
...
- **Color**: Output intended for *Printer*, with the printer's ICC profile if you have it (put `.icc` files in `~/.local/share/icc` and restart Scribus).

## 5. Check before sending
```bash
bash scripts/pdf-preflight.sh event-flyer.pdf      # boxes, fonts, image ppi
bash scripts/pdf-proof.sh event-flyer.pdf          # PNG proofs for you/the client
```
