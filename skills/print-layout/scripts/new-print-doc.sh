...
# Usage: new-print-doc.sh <preset> <name> [--pages N] [--title "Text"] [--font "IBM Plex Sans"] [--proof]
...
preset=${1:-}; name=${2:-}
[[ -n $preset && -n $name ]] || { sed -n '2,6p' "$0"; exit 2; }
shift 2
pages=1; title="Headline goes here"; font=""; proof=0
while (($#)); do
  case $1 in
    --pages) pages=$2; shift ;;
    --title) title=$2; shift ;;
...
DOC_OUT=$sla DOC_PDF=$pdf DOC_TITLE=$title DOC_FONT=$font \
  timeout 120 scribus -g -ns -py "$(dirname "$(readlink -f "$0")")/scribus_new_doc.py" >/dev/null 2>&1 || true
