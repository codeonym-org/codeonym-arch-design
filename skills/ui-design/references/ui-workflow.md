...
1. **Color**: brand scale via `bash scripts/palette.sh '#6C63FF' brand`, plus a neutral scale (`palette.sh '#64748b' gray`), success/warning/danger. Import the hex values as document colors (Lunacy: *Colors* panel; Penpot: *Assets → Colors*).
...
- Export tokens: `palette.sh` already emits a Tailwind v4 `@theme` block — paste into `app/globals.css`.
...
- Favicons & app icons: `bash scripts/icon-set.sh logo-mark.svg public --bg '#0d1117' --name 'My App'`.
